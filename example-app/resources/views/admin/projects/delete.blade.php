...
@section('content')
    ...
    <form id="form" action="{{ route('projects.destroy', ['project' => $project->id])}}" method="POST">
        @method('DELETE')
        @csrf
        <label for="name">Name</label>
        <input name="name" id="name" type="text" value="{{ ( $project->name) }}" disabled><br>
        <label>Description</label>
        <input name="description" id="name" type="text" value="{{ ( $project->description) }}" disabled>
